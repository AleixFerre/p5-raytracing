...
namespace Custom {
  ...
  export class Ray {
    ...
    draw(walls: Custom.Wall[], depth: number = 0) {
      stroke('red');
      strokeWeight(10);
    ...
    private calculateIntersectionPoint(walls: Custom.Wall[]): IntersectionPointInfo {
    ...
    private getReflectionDirection(direction: p5.Vector, wall: Custom.Wall): p5.Vector {
    ...
    private getRayToLineSegmentIntersection(wall: Custom.Wall): p5.Vector | null {
      const x1 = wall.p1.x;
      const y1 = wall.p1.y;
      const x2 = wall.p2.x;
      const y2 = wall.p2.y;
